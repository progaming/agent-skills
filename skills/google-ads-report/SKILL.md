...
> **Note:** Google deprecated the `urn:ietf:wg:oauth:2.0:oob` (out-of-band) flow. Use `http://localhost:8085` as the redirect URI instead.

**Step 1:** Generate the authorization URL and open it in a browser:

```bash
echo "https://accounts.google.com/o/oauth2/v2/auth?client_id=${GOOGLE_CLIENT_ID}&redirect_uri=http://localhost:8085&scope=https://www.googleapis.com/auth/adwords&response_type=code&access_type=offline&prompt=consent"
```

**Step 2:** After signing in and granting permission, the browser will redirect to `http://localhost:8085/?code=...&scope=...`. Copy the `code` value from the address bar (you may see a "This site can't be reached" error — that's expected).
...
> **Note:** The endpoint format is `v{VERSION}/customers/{CUSTOMER_ID}/googleAds:searchStream` (note the **`/googleAds`** path segment). As of mid-2026, **v24** is the current stable version. Earlier versions and the colon-only format (`:searchStream` without `/googleAds`) return `404`.
...
- **Encoding:** On Windows, set `sys.stdout.reconfigure(encoding='utf-8')` before printing non-English keywords (e.g., Thai, Chinese) to avoid `UnicodeEncodeError`
